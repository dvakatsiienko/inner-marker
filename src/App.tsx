...
const NavLink = ({
  ...
  return (
    <RRNavLink
      className={({ isActive }) =>
        cx(isActive ? 'text-amber-600' : '', className)
      }
      end
      to={to}>
      {children}
  ...
  return (
    <Card className='flex flex-col gap-2 px-2 [grid-area:sidebar-l]'>
      ...
      <div className='flex flex-col gap-2'>
        <NavLink className={navLinkCn} to='/dashboard'>
          Dashboard
        </NavLink>
        <NavLink className={navLinkCn} to='/dashboard/settings'>
          Settings
        </NavLink>
      </div>
      <Separator className={separatorCn} />
...
export function App() {
  return (
    <main className='min-h-screen grid gap-x-16 gap-y-2 auto-rows-min p-4 layout'>
      <Links />
      ...
      <Routes>
        ...
        <Route
          ...
          path='projects'>
          <Route element={<ProjectsIndex />} index />
          <Route element={<ProjectLayout />}>
            <Route element={<Project />} path=':pid' />
            <Route element={<EditProject />} path=':pid/edit' />
          </Route>
        </Route>
        ...
        <Route path='events'>
          <Route
            element={
              <h1 className='heading [grid-area:content]'>Events Index</h1>
            }
            index
          />
          <Route element={<EventsOptional />} path=':city' />
          <Route
        ...
        <Route
          ...
          <Route
            element={
              <h1 className='heading [grid-area:content]'>Test Index</h1>
            }
            index
          />
          <Route element={<TestOptParam />} path='files/:testId' />
        </Route>

        <Route
          element={<TodoList todoList={todoListStore1.todoList} />}
          path='mobx-1'
        />
        <Route
          element={<TodoList todoList={todoListStore2.todoList} />}
          path='mobx-2'
        />
        <Route
          ...
          path='mobx-3'
        />
        <Route element={<span>todo</span>} path='zustand' />
      </Routes>
    </main>
  );
}
